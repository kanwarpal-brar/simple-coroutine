...
class CorJob  {
  ...
  private:
    ...
    CorJob(void * stk_ptr, std::size_t size, ctx::stack_context sctx, StackAllocator salloc): cont{} {
        cont=ctx::callcc(
          std::allocator_arg,
    ...
    void init_stack() {
        ...
        // place new mirror CorJob on stack
        companion=new(stk_ptr)CorJob(stk_ptr, size, sctx, salloc);
    }
